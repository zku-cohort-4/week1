...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            ...
            puzzleRangeProof[i][j].out === 1; //[assignment] change assert() to use your created RangeProof instead
            solutionRangeProof[i][j].out === 1; //[assignment] change assert() to use your created RangeProof instead
